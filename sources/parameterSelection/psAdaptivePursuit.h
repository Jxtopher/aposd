	...
	void _selection_probability() {
		...
		for (unsigned int i = 1; i < this->_number_of_parameters ; i++) {
			if (reward_estimate_max < reward_estimate[i])
				reward_estimate_max = reward_estimate[i];
		}

		for (unsigned int i = 0; i < this->_number_of_parameters ; i++) {
			if (reward_estimate_max == reward_estimate[i]) {
				selection_probability[i] = selection_probability[i] + _beta * (_p_max - selection_probability[i]);
			} else {
				selection_probability[i] = selection_probability[i] + _beta * (_p_min - selection_probability[i]);
			}
		}
	}
