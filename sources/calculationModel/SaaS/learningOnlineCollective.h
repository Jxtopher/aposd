...
public:
    ...
    std::pair<SOL, unsigned int> initial_solution(const SOL &s) {
        ...
        if (solution_star_init == false) {
            ...
            if (_selection->operator()(s, solution_star)) {
                solution_star = s;
            }
        }
    ...
    std::pair<SOL, unsigned int> run(const SOL &s_t0, const SOL s_t1, unsigned int parameter) {
        ...
        std::pair<double, unsigned int> rewardOp = _rewardComputation->operator()(s_t0, s_t1, parameter);
        std::pair<double, unsigned int> delta_f = _rewardComputation->operator()(s_t1, solution_star, parameter);
        if (delta_f.first < 0) { // Negatif
            double average = 0;
            if (history_fitness.size() != 0)
                average = std::accumulate(history_fitness.begin(), history_fitness.end(), 0.0) / history_fitness.size();
        ...
        if (_selection->operator()(s_t1, solution_star)) {
            solution_star = s_t1;
        }
