...
PsSelectBestMutate::PsSelectBestMutate(std::shared_ptr<std::mt19937> mt_rand, unsigned int number_of_parameters,
...
PsSelectBestMutate::PsSelectBestMutate(const PsSelectBestMutate& c)
...
PsSelectBestMutate::~PsSelectBestMutate() {}

ParameterSelection* PsSelectBestMutate::clone() const { return new PsSelectBestMutate(*this); }

void PsSelectBestMutate::reset() {
...
void PsSelectBestMutate::update(std::vector<std::pair<double, unsigned int>>& rewards) {
	if(_aggregation_function == AggregationFunction::MAX) {
		unsigned int max_index = 0;
		unsigned int min_index = 0;
		for(unsigned i = 1; i < rewards.size(); i++) {
			if(rewards[max_index].first < rewards[i].first) max_index = i;
			if(rewards[i].first < rewards[min_index].first) min_index = i;
		}
		...
		for(unsigned i = 1; i < rewards.size(); i++) {
			reward_aggregation[rewards[i].second] =
				(reward_aggregation[rewards[i].second] * number_select[rewards[i].second] +
					rewards[i].first) /
				(number_select[rewards[i].second] + 1);
...
void PsSelectBestMutate::update(std::pair<double, unsigned int>& rewards) {
...
std::vector<unsigned int> PsSelectBestMutate::getParameter(const unsigned int number_of_nodes) {
...
unsigned int PsSelectBestMutate::getParameter() {
...
std::string PsSelectBestMutate::className() const { return "PsSelectBestMutate"; }
