...
PsUCBW::PsUCBW(std::shared_ptr<std::mt19937> mt_rand, unsigned int number_of_parameters,
		const double C, const unsigned int windowSize,
...
ParameterSelection* PsUCBW::clone() const { return new PsUCBW(*this); }
...
void PsUCBW::update(std::vector<std::pair<double, unsigned int>>& rewards) {
...
///
void PsUCBW::update(std::pair<double, unsigned int>& rewards) {
...
std::vector<unsigned int> PsUCBW::getParameter(const unsigned int number_of_nodes) {
...
unsigned int PsUCBW::getParameter() {
	for(unsigned int i = 0; i < this->_number_of_parameters; i++) {
		if(Q[i] == 0) return i;
	}
...
std::string PsUCBW::className() const { return "PsUCBW"; }