...
template<class SOL>
MasterSynchronous<SOL>::MasterSynchronous(std::unique_ptr<ParameterSelection> parameterSelection,
...
template<class SOL>
MasterSynchronous<SOL>::~MasterSynchronous() {}

template<class SOL>
void MasterSynchronous<SOL>::operator()() {
	...
	for(int i = 1; i < mpi_globals_number_of_nodes; i++) {
		int sizeOfmessage;
		MPI_Probe(i, MPI_TAG, MPI_COMM_WORLD,
					&status); // It will block the caller until a message is ready
		MPI_Get_count(&status, MPI_CHAR, &sizeOfmessage); // Obtain message size
		char* msg = new char[sizeOfmessage + 1];
		MPI_Recv(msg, sizeOfmessage, MPI_CHAR, i, MPI_TAG, MPI_COMM_WORLD,
					&status); // Finally, receive the message with a correctly sized buffe
	...
	do {
		...
		std::vector<unsigned int> parameter =
			_parameterSelection->getParameter(mpi_globals_number_of_nodes - 1);
		...
		for(int i = 1; i < mpi_globals_number_of_nodes; i++) {
			...
			MPI_Isend((char*)soluString.c_str(), soluString.size(), MPI_CHAR, i, MPI_TAG,
		...
		std::vector<std::pair<double, unsigned int>> rewardOp =
			_rewardComputation->operator()(solutions, solutionsAfterMutation, parameter);
