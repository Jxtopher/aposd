...
template<class SOL> 
SharedParameter<SOL>::SharedParameter(int argc, char** argv, std::unique_ptr<Topologies> topologies,
	...
	: IslandModel<SOL>(argc, argv, std::move(topologies)), _launcher(std::move(launcher)),
...
template<class SOL> 
SharedParameter<SOL>::~SharedParameter() { delete recv_msg; }

template<class SOL> 
void SharedParameter<SOL>::operator()() {
	...
	do {
		...
		std::pair<double, unsigned int> rewardOp =
			_rewardComputation->operator()(bestSolution, solutions.back(), parameter);
		...
		for(unsigned int neighbour : make_iterator_range(neighbours)) {
			...
			// Send solution
			MPI_Isend(static_cast<const char*>(newSolution.c_str()), newSolution.size(),
						MPI_CHAR, neighbour, MPI_TAG, MPI_COMM_WORLD, &request);

			// Send reward
			MPI_Isend(&(rewardOp.first), 1, MPI_DOUBLE, neighbour, MPI_TAG, MPI_COMM_WORLD,
						&request);

			// Send parameter
			MPI_Isend(&(rewardOp.second), 1, MPI_INT, neighbour, MPI_TAG, MPI_COMM_WORLD,
						&request);
		}
		...
		for(unsigned int neighbour : make_iterator_range(neighbours)) {
			...
			switch(order) {
				...
				case MPI_Order::COMPUTE_FITNESS: {
					...
					MPI_Recv(recv_msg, recv_sizeOfmessage, MPI_CHAR, status.MPI_SOURCE, MPI_TAG,
								MPI_COMM_WORLD, &status); // Get solution

					MPI_Recv(&recv_reward, 1, MPI_DOUBLE, status.MPI_SOURCE, MPI_TAG,
								MPI_COMM_WORLD, &status); // Get reward

					MPI_Recv(&recv_parameter, 1, MPI_INT, status.MPI_SOURCE, MPI_TAG,
								MPI_COMM_WORLD, &status); // Get parameter
